...
app.get("/years", async (req, res) => {
  try {
    ...
    for(let i = 0; i < data.length; i++) {
      type.add(...(String(data[i]["Year"]).split('–')));
    }
...
app.post("/movies", async (req, res) => {
    const {type, genre, year, name} = req.body;  

    try {
      const newDataYear = data.filter(el => {
        
        const yearr = el.Year.split("–");
        if(yearr.length == 1) {
          if( Number(yearr[0]) >= Math.min(year[0], year[1]) && Number(yearr[0]) <= Math.max(year[0], year[1]) ) {
            return true;
          } return false;
        }

        if(year[1] == '') {
          if( Number(yearr[0]) <= Math.max(year[0], year[1]) ) {
            return true;
          } return false;
        }

        if( Number(yearr[0]) >= Math.min(year[0], year[1]) &&  Number(yearr[1]) <= Math.max(year[0], year[1])) {
          return true;
        } return false;
      
      });
      const newDataType = type.length != 0 ? newDataYear.filter(el => type.includes(el.Type)) : newDataYear;
      const newDataGenre = genre.length != 0 ? newDataType.filter(el => {
        const arr = el.Genre.split(", ");
        for(let i = 0; i < genre.length; i++) {
          if(!arr.includes(genre[i])) {
            return false;
          }
        }
        return true;
...
app.post("/movie", async (req, res) => {
  const {id} = req.body;
  try {
    const movie = data.filter(el => el.imdbID == id);
    res.json(movie[0]);
  } catch (err) {
    console.error(err.message);
  }
